...
const wheaterapi_api_key = process.env.wheaterapi_api_key;
...
const handleLocation = async (msg) => {
  ...
  const weather_endpoint = (timestamp='') => {
    const r = `http://api.weatherapi.com/v1/forecast.json?key=${wheaterapi_api_key}&q=${latitude},${longitude}&unixdt=${timestamp}&days=${2}&alerts=yes`;
    console.log(r)
    return r;
  };
  const todayData = await axios.get(weather_endpoint())
  const yesterdayData = await axios.get(weather_endpoint(yesterday))
